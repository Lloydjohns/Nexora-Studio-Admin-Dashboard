...
import { KpiCard, StatusBadge, Avatar, PriorityBadge, ProgressBar } from '@/components/shared';
...
import { fetchProjects, insertProject, updateProject, deleteProject } from '@/lib/api';
...
const serviceTypeOptions = ['Social Media', 'Web Development', 'Web + Systems', 'Social + Design', 'Design', 'Social + Strategy'];
...
export default function ProjectsPage() {
  ...
  const { data: projects, loading } = useFetch(fetchProjects, [refreshKey]);
  ...
  const inProgress = allProjects.filter((p) => p.stage !== 'Completed').length;
  const completed = allProjects.filter((p) => p.stage === 'Completed').length;
  const highPriority = allProjects.filter((p) => p.priority === 'High' && p.stage !== 'Completed').length;
  ...
  return (
    <DashboardShell>
      <PageHeader title="Projects" description="Track project delivery across all stages">
      ...
      <div className="mt-6 grid grid-cols-2 gap-4 lg:grid-cols-4">
        <KpiCard label="Total Projects" value={String(allProjects.length)} icon={FolderKanban} index={0} />
        <KpiCard label="In Progress" value={String(inProgress)} delta="+3" trend="up" icon={TrendingUp} accent="text-amber-600 bg-amber-100 dark:bg-amber-500/15 dark:text-amber-400" index={1} />
        <KpiCard label="Completed" value={String(completed)} delta="+1" trend="up" icon={Clock} accent="text-emerald-600 bg-emerald-100 dark:bg-emerald-500/15 dark:text-emerald-400" index={2} />
        <KpiCard label="High Priority" value={String(highPriority)} icon={AlertCircle} accent="text-rose-600 bg-rose-100 dark:bg-rose-500/15 dark:text-rose-400" index={3} />
      </div>
                ...
                return (
                  <div key={stage} className="w-72 shrink-0">
                    <div className="mb-2 flex items-center gap-2 px-1">
                      ...
                      <Badge variant="secondary" className="ml-auto text-[10px]">{stageProjects.length}</Badge>
                    </div>
                    <div className="space-y-2">
                      {stageProjects.map((p, i) => (
                        ...
                        >
                          <Card className="cursor-pointer transition-shadow hover:shadow-md" onClick={() => openEdit(p.id)}>
                            <CardContent className="p-3.5">
                              <div className="flex items-start justify-between gap-2">
                                <p className="text-sm font-semibold leading-snug">{p.name}</p>
                                <Trash2 className="h-3.5 w-3.5 shrink-0 text-muted-foreground hover:text-rose-500" onClick={(e) => { e.stopPropagation(); handleDelete(p.id); }} />
                              </div>
                              ...
                              <div className="mt-3 flex items-center justify-between">
                                <div className="flex -space-x-1.5">
                                  {p.team.slice(0, 3).map((m) => (
                                    <Avatar key={m} initials={getInitials(m)} className="h-6 w-6 border-2 border-card text-[9px]" />
                                  ))}
                                  {p.team.length > 3 && (
                ...
                {loading ? (
                  ...
                  allProjects.map((p) => (
                    <TableRow key={p.id} className="cursor-pointer" onClick={() => openEdit(p.id)}>
                      ...
                      <TableCell className="text-muted-foreground text-sm">{p.deadline}</TableCell>
                    </TableRow>
                  ))
                )}
              </TableBody>
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Select value={form.serviceType} onValueChange={(v) => setForm({ ...form, serviceType: v })}>
                  ...
                  <SelectContent>
                    {serviceTypeOptions.map((s) => <SelectItem key={s} value={s}>{s}</SelectItem>)}
                  </SelectContent>
                </Select>
              </div>
              <div className="space-y-2">
              ...
              <div className="space-y-2 sm:col-span-2">
                <Label htmlFor="team">Team (comma-separated names)</Label>
                <Input id="team" value={form.team} onChange={(e) => setForm({ ...form, team: e.target.value })} placeholder="Andrea Lim, Kai Santos" />
              </div>
            </div>
            <DialogFooter>
              {editId && (
              ...
              <Button type="submit" disabled={submitting}>{submitting ? 'Saving...' : editId ? 'Update' : 'Create Project'}</Button>
            </DialogFooter>
          </form>
        </DialogContent>
      </Dialog>
